...
const parseEntityVector3 = (value, fallback = [0, 0, 0]) => {
...
const parseEntityAngles = entity => {
  if (typeof entity?.angles === 'string') {
    return parseEntityVector3(entity.angles)
  }

  const yawOnly = Number(entity?.angle)
  if (Number.isFinite(yawOnly)) {
    return [0, yawOnly, 0]
  }
...
const parseBspTeleportMetadata = ({ mapName, bspPath, bspSource }) => {
  const data = readBspData(bspPath)
  ...
  const modelLump = readLump(LUMP_MODELS)
  const models = []
  const modelStride = 48
  for (
    let modelOffset = 0;
    modelOffset + modelStride <= modelLump.length;
    modelOffset += modelStride
  ) {
    models.push({
      mins: [
        modelLump.readFloatLE(modelOffset + 0),
        modelLump.readFloatLE(modelOffset + 4),
        modelLump.readFloatLE(modelOffset + 8),
      ],
      maxs: [
        modelLump.readFloatLE(modelOffset + 12),
        modelLump.readFloatLE(modelOffset + 16),
        modelLump.readFloatLE(modelOffset + 20),
      ],
      headnode: modelLump.readInt32LE(modelOffset + 36),
    })
  }
  ...
  for (let planeOffset = 0; planeOffset + 20 <= planeLump.length; planeOffset += 20) {
    planes.push({
      normal: [
        planeLump.readFloatLE(planeOffset + 0),
        planeLump.readFloatLE(planeOffset + 4),
        planeLump.readFloatLE(planeOffset + 8),
      ],
      dist: planeLump.readFloatLE(planeOffset + 12),
    })
  }
  ...
  for (let nodeOffset = 0; nodeOffset + 32 <= nodeLump.length; nodeOffset += 32) {
    nodes.push({
      children0: nodeLump.readInt32LE(nodeOffset + 4),
      children1: nodeLump.readInt32LE(nodeOffset + 8),
    })
  }
  ...
  for (let leafOffset = 0; leafOffset + 32 <= leafLump.length; leafOffset += 32) {
    leafs.push({
      firstLeafBrush: leafLump.readUInt16LE(leafOffset + 24),
      numLeafBrushes: leafLump.readUInt16LE(leafOffset + 26),
    })
  }

  const leafBrushLump = readLump(LUMP_LEAFBRUSHES)
  const leafBrushes = []
  for (let index = 0; index + 2 <= leafBrushLump.length; index += 2) {
    leafBrushes.push(leafBrushLump.readUInt16LE(index))
  }

  const brushLump = readLump(LUMP_BRUSHES)
  const brushes = []
  for (let brushOffset = 0; brushOffset + 12 <= brushLump.length; brushOffset += 12) {
    brushes.push({
      firstSide: brushLump.readInt32LE(brushOffset + 0),
      numSides: brushLump.readInt32LE(brushOffset + 4),
      contents: brushLump.readInt32LE(brushOffset + 8),
    })
  }

  const brushSideLump = readLump(LUMP_BRUSHSIDES)
  const brushSides = []
  for (let sideOffset = 0; sideOffset + 8 <= brushSideLump.length; sideOffset += 8) {
    brushSides.push({
      planeNum: brushSideLump.readUInt16LE(sideOffset + 0),
    })
  }

  const collectBrushIndicesForModel = headnode => {
    const pendingNodes = [headnode]
    const visitedNodes = new Set()
    const visitedLeafs = new Set()
    ...
    while (pendingNodes.length > 0) {
      ...
      if (nodeIndex < 0) {
        const leafIndex = -nodeIndex - 1
        if (leafIndex < 0 || leafIndex >= leafs.length || visitedLeafs.has(leafIndex)) {
          continue
        }
        visitedLeafs.add(leafIndex)
        const leaf = leafs[leafIndex]

        for (let leafBrushOffset = 0; leafBrushOffset < leaf.numLeafBrushes; leafBrushOffset += 1) {
          const listIndex = leaf.firstLeafBrush + leafBrushOffset
          if (listIndex < 0 || listIndex >= leafBrushes.length) continue
          brushIndices.add(leafBrushes[listIndex])
        }

        continue
      }

      if (nodeIndex >= nodes.length || visitedNodes.has(nodeIndex)) continue
      visitedNodes.add(nodeIndex)

      const node = nodes[nodeIndex]
      pendingNodes.push(node.children0)
      pendingNodes.push(node.children1)
    }

    return Array.from(brushIndices)
  }

  const collectTriggerBrushPlanes = modelIndex => {
    const model = models[modelIndex]
    if (!model) return []

    const brushIndices = collectBrushIndicesForModel(model.headnode)
    const triggerBrushes = []

    for (const brushIndex of brushIndices) {
      if (!Number.isInteger(brushIndex) || brushIndex < 0 || brushIndex >= brushes.length) continue

      const brush = brushes[brushIndex]
      if (brush.numSides <= 0) continue

      const planesForBrush = []
      for (let sideOffset = 0; sideOffset < brush.numSides; sideOffset += 1) {
        const sideIndex = brush.firstSide + sideOffset
        if (sideIndex < 0 || sideIndex >= brushSides.length) continue

        const brushSide = brushSides[sideIndex]
        if (brushSide.planeNum < 0 || brushSide.planeNum >= planes.length) continue

        const plane = planes[brushSide.planeNum]
        planesForBrush.push({
          normal: plane.normal,
          dist: plane.dist,
        })
      ...
      if (planesForBrush.length > 0) {
        triggerBrushes.push({
          planes: planesForBrush,
        })
      }
    }
  ...
  const entitiesText = readLump(LUMP_ENTITIES).toString('latin1')
  const entityBlocks = entitiesText.match(/\{[\s\S]*?\}/g) ?? []

  const parseEntity = block => {
    const fields = {}
    const fieldMatches = block.match(/"[^"]+"\s+"[^"]*"/g) ?? []
  ...
  const entities = entityBlocks.map(parseEntity)

  const destinations = new Map()
  for (const entity of entities) {
    const className = String(entity.classname || '').toLowerCase()
    if (className !== 'info_teleport_destination') continue

    const targetName = String(entity.targetname || '').trim()
    if (!targetName) continue

    destinations.set(targetName, {
      origin: parseEntityVector3(entity.origin),
      angles: parseEntityVector3(entity.angles),
    })
  }

  const roundVec = values => values.map(component => Number(Number(component).toFixed(5)))
  const roundNumber = value => Number(Number(value).toFixed(5))

  const spawnClassNames = new Set([
  ...
  for (const entity of entities) {
    const className = String(entity.classname || '').toLowerCase()
    if (!spawnClassNames.has(className)) continue

    const origin = parseEntityVector3(entity.origin, [Number.NaN, Number.NaN, Number.NaN])
    if (origin.some(component => !Number.isFinite(component))) continue

    const angles = parseEntityAngles(entity)
    const rawTeamNum = entity.TeamNum ?? entity.teamnum ?? entity.teamNum
    const parsedTeamNum = Number(rawTeamNum)
    const teamNum = Number.isFinite(parsedTeamNum) ? Math.trunc(parsedTeamNum) : null

    spawns.push({
      className,
      teamNum,
      origin: roundVec(origin),
      angles: roundVec(angles),
    })
  }

  const defaultSpawn =
    spawns.find(spawn => spawn.teamNum === 3) ??
    spawns.find(spawn => spawn.teamNum === 2) ??
    spawns[0] ??
    null
  ...
  for (const entity of entities) {
    const className = String(entity.classname || '').toLowerCase()
    if (className !== 'trigger_teleport') continue

    const modelValue = String(entity.model || '')
    if (!modelValue.startsWith('*')) {
      missingModelCount += 1
      continue
    }

    const modelIndex = Number(modelValue.slice(1))
    if (!Number.isInteger(modelIndex) || modelIndex < 0 || modelIndex >= models.length) {
      missingModelCount += 1
      continue
    }

    const targetName = String(entity.target || '').trim()
    const destination = destinations.get(targetName)
    if (!destination) {
    ...
    const model = models[modelIndex]
    const triggerOrigin = parseEntityVector3(entity.origin)

    const mins = [
      model.mins[0] + triggerOrigin[0],
      model.mins[1] + triggerOrigin[1],
      model.mins[2] + triggerOrigin[2],
    ]
    const maxs = [
      model.maxs[0] + triggerOrigin[0],
      model.maxs[1] + triggerOrigin[1],
      model.maxs[2] + triggerOrigin[2],
    ]

    const triggerBrushes = collectTriggerBrushPlanes(modelIndex)

    triggers.push({
      id: `teleport_${modelIndex}`,
      targetName,
      modelIndex,
      mins: roundVec(mins),
      maxs: roundVec(maxs),
      origin: roundVec(triggerOrigin),
      brushes: triggerBrushes.map(brush => ({
        planes: brush.planes.map(plane => ({
          normal: roundVec(plane.normal),
          dist: roundNumber(plane.dist),
        })),
      })),
      destination: {
        origin: roundVec(destination.origin),
        angles: roundVec(destination.angles),
      },
    })
  }

  triggers.sort((left, right) => left.modelIndex - right.modelIndex)
...
let teleportsMetadata = null
const teleportsMetadataPath = path.join(outDir, 'teleports.json')
try {
  teleportsMetadata = parseBspTeleportMetadata({
    mapName,
    bspPath,
    bspSource: sourceBspPath,
  })
  fs.writeFileSync(teleportsMetadataPath, JSON.stringify(teleportsMetadata, null, 2))
  console.log(
    `Teleport metadata: ${teleportsMetadataPath} (${teleportsMetadata.triggerCount} triggers)`
  )
} catch (error) {
  console.warn(
    `Teleport metadata extraction failed: ${error instanceof Error ? error.message : error}`
  )
}
...
const conversionMeta = {
  ...
  skyboxImageFormat,
  allowSkyboxFallback,
  teleports: teleportsMetadata
    ? {
        triggerCount: teleportsMetadata.triggerCount,
        spawnCount: teleportsMetadata.spawnCount,
        hasDefaultSpawn: Boolean(teleportsMetadata.defaultSpawn),
        missingModelCount: teleportsMetadata.missingModelCount,
